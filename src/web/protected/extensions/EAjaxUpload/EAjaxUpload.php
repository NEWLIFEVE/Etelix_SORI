...
class EAjaxUpload extends CWidget
{
  ...
  public function run()
  {
    //primero verificar que todo este en orden
    if(empty($this->config['action']))
		{
      throw new CException('EAjaxUpload: Parametro "action" no puede estar vacío.');
    }

		if(empty($this->config['allowedExtensions']))
		{
      throw new CException('EAjaxUpload: Parametro "allowedExtensions" no puede estar vacío.');
    }

		if(empty($this->config['sizeLimit']))
		{
      throw new CException('EAjaxUpload: Parametro "sizeLimit" no puede estar vacío.');
    }

    unset($this->config['element']);
    echo '<div id="'.$this->id.'"><noscript><p>Por favor activar JavaScript para utilizar la subida de archivos.</p></noscript></div>';
