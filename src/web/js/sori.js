            	...
            	case "editTP":  case "saveTP":  case "cancelTP": case "deleteTP": 
                    ...
                    switch ($(this).attr('name'))
                    {
                    	case "editTP":
                            ...
                            break; 
                        case "saveTP":
                        	$SORI.AJAX.actualizar($fila[0].id, type); 
                        	_revert_TP($fila,type); 
                            break; 
                        case "cancelTP":
                        	_revert_TP($fila,type);
                            break; 
                        case "deleteTP": 
                        	_elimina_doc($fila, type);
                            break;
                    }
                    break;
            }
...
//                var respuesta=$SORI.UI.validaCampos($('#AccountingDocumentTemp_carrier_groups,#AccountingDocumentTemp_valid_received_date,#AccountingDocumentTemp_doc_number,#AccountingDocumentTemp_amount').serializeArray());
	...
	{  
        ...
        if(respuesta==0)
        {
        	...
        	$SORI.UI.msj_change("<h3>Faltan datos por agregar</h3>","aguanta.png","1000","width:40px; height:90px;");
        }
        return respuesta;
    }
...
$SORI.AJAX=(function()
{
	...
	function borrar(id, extra)
	{
        ...
        {
        	case true:
        		var url = "/ContratoTerminoPagoSupplier/DeleteCTPS/"+id;console.log(extra);
                break;
            case false:
            	var url = "/ContratoTerminoPago/DeleteCTP/"+id;console.log(extra);
                break;
            case null: 
            	var url = "/AccountingDocumentTemp/borrar/"+id;console.log(extra);
                break;
        }
		$.ajax(
		{
...
//				console.log('actualizo');
    ...
    {
    	$.ajax({
    		url:"../TerminoPago/Names",success:function(datos)
	        {
	            ...
	            $(""+obj+"").autocomplete({source:$SORI.DATA.names});
